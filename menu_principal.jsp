<%-- 
    ...
    Created on : 7 oct. 2021, 10:13:14
    Author     : Usuario
--%>
    ...
    <body >
        ...
        <main>
            <div class="container-fluid" style="height: 100%;">
                <h1 class="text-center" style="color: rgb(255,255,255);font-size: 32px;margin-top: 5px;margin-bottom: 25px;">Notas Recientes</h1>
            </div>
             <%
           int i=0;
            ArrayList<NotaDTO> notas = (ArrayList<NotaDTO>)request.getAttribute("Notas");
            for(NotaDTO actual : notas){
                if(actual.isEliminada()==false){
                if(i==0){
                out.print("<div class=\"container\" id=\"1\" style=\"margin-top: 30px;margin-bottom: 21px;\">"
                        +"<div class=\"row\">"
                        +"<div class=\"col-md-6\">"
                        +"<div class=\"card-body\" style=\"color: rgb(255,255,255);box-shadow: 15px 15px 20px rgba(0,0,0,0.3);border-style: none;border-color: #343a40;\">"
                        +"<h4 class=\"card-title\">" + actual.getTitulo() +"</h4>"
                                +"<h6 class=\"text-muted card-subtitle mb-2\">" +actual.getCreación() + "<br></h6> "
                                        +"<p class=\"card-text\">" + actual.getNota().replaceAll("#([A-Za-z]+)", "<a href=\"/PIAprograweb/ServlettNota?hashtag=$1\">#$1</a>") + "</p>"
                                                + "<form action=\"/PIAprograweb/ServletNota\" method=\"POST\">"
                                                + "<input type=\"hidden\" name=\"id\" value=\"" + actual.getId() + "\"> " 
                                                        +"<input type=\"hidden\" name=\"accion\" value=\"eliminar\">"
                                                        + "<input type=\"submit\" class=\"btn btn-primary btn-block btn-sm\" style=\"background: #4d84bb;color: #ffffff;width: 100%;margin-top: 5px; padding: 10px;\" value=\"Eliminar\">"
                                                        + "</form>"
                                                        +"<form action=\"ActualizarNota.jsp\" method\"GET\">"
                                                        +"<input type=\"hidden\" name=\"id\" value=\"" + actual.getId() + "\">"
                                                                +"<input type=\"hidden\" name=\"Nota\" value=\"" + actual.getNota() + "\"> "
                                                                        +"<input type=\"hidden\" name=\"Titulo\" value=\"" + actual.getTitulo()+ "\"> "
                                                                                +"<input type=\"submit\" class=\"btn btn-primary btn-block btn-sm\" style=\"background: #4d84bb;color: #ffffff;width: 100%;margin-top: 0px;   padding: 10px; \" value=\"Modificar\">"
                                                        + "</form>"
                        +"</div>" 
                        +"</div>");
                i=1;
                     ...
                     out.print("<div class=\"col-md-6\">"
                        + "<div class=\"card-body\" style=\"color: rgb(255,255,255);box-shadow: 15px 15px 20px rgba(0,0,0,0.3);\">"
                       +"<h4 class=\"card-title\">" + actual.getTitulo() +"</h4>"
                                +"<h6 class=\"text-muted card-subtitle mb-2\">" +actual.getCreación() + "<br></h6> "
                                        +"<p class=\"card-text\">" + actual.getNota().replaceAll("#(.+)", "<a href=\"/PIAprograweb/ServlettNota?hashtag=$1\">#$1</a>") + "</p>"
                                                 + "<form action=\"/PIAprograweb/ServletNota\" method=\"POST\">"
                                                + "<input type=\"hidden\" name=\"id\" value=\"" + actual.getId() + "\"> " 
                                                        +"<input type=\"hidden\" name=\"accion\" value=\"eliminar\">"
                                                        + "<input type=\"submit\" class=\"btn btn-primary btn-block btn-sm\" style=\"background: #4d84bb;color: #ffffff;width: 100%;margin-top: 5px; padding: 10px;\" value=\"Eliminar\">"
            ...
            <nav class="d-none d-lg-flex justify-content-lg-center" style="margin-bottom: 20px;margin-top: 50px;color: rgb(41,38,33);">
                <ul class="pagination pagination-sm">
                    <li class="page-item"><a class="page-link" href="/PIAprograweb/ServletNota?pagina=${param["pagina"]-1}" aria-label="Anterior"><span aria-hidden="true">«</span></a></li>
                    <li class="page-item"><a class="page-link" href="/PIAprograweb/ServletNota?pagina=${param["pagina"]+1}" aria-label="Siguiente"><span aria-hidden="true">»</span></a></li>
                </ul>
            </nav>
